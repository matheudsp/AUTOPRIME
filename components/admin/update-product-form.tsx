...
const UpdateProductForm = ({ vehicleId, vehicle }: UpdateVehicleFormProps) => {
  ...
  const form = useForm<z.infer<typeof newVehicleSchema>>({
    ...
    defaultValues: {
      ...
      discountPercentage: String(vehicle.discountPercentage),
      specialTag: vehicle.specialTag,
    },
  });
  ...
  const handleSubmit = async (data: z.infer<typeof newVehicleSchema>) => {
    try {
      ...
      await updateVehicle({ data: vehicleData, vehicleId });
      ...
      toast.error(`Ocorreu um erro ao atualizar o veiculo! ${error}` , {
        style: {
          fontSize: "12px",
        },
      });
